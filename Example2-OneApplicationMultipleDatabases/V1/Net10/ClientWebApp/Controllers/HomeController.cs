    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> ViewServiceData(string create)
        {
            ...
            if (!string.IsNullOrEmpty(create))
                await CreateAnExampleRecord(create);
            ...
            // Query work microservice
            var respProcess = await _processApiClient.QueryAsync(defaultQuery);
            model.Processes = respProcess.Item.List;

            return View("ViewServiceData", model);
        }

        private async Task CreateAnExampleRecord(string create)
        {
            switch (create)
            {
                default:
                ...
                case "security":
                    ...
                    // Find a user
                    var respUsers = await _userApiClient.QueryAsync(ServiceQueryRequestBuilder.New().Build());
